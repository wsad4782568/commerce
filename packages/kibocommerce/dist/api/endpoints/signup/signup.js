...
const signup = async ({ body: { email , password , firstName , lastName  } , config ,  })=>{
    let response;
    try {
        var ref, ref1;
        // Register user
        ...
        if (error instanceof FetcherError && invalidCredentials.test(error.message)) {
            throw new CommerceAPIError("Cannot find an account that matches the provided credentials", {
                status: 401,
