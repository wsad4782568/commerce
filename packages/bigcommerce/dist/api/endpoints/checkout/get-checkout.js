...
const getCheckout = async ({ req , config ,  })=>{
    ...
    const customerToken = (ref1 = cookies.get(config.customerCookie)) == null ? void 0 : ref1.value;
    if (!cartId) {
        return {
            redirectTo: "/cart"
        };
    ...
    if (!customerId) {
        ...
        const { uuid  } = process.env.NEXT_RUNTIME === "edge" ? await import("@cfworker/uuid") : await import("uuidv4");
        const jwt = process.env.NEXT_RUNTIME === "edge" ? await import("@tsndr/cloudflare-worker-jwt") : await import("jsonwebtoken");
